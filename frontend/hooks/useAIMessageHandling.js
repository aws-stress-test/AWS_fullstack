...
export const useAIMessageHandling = (
  ...
  const handleAIMessageChunk = useCallback(
    (data) => {
      if (!data.messageId) {
      ...
      setStreamingMessages((prev) => {
        ...
        if (!prev[data.messageId]) {
          console.warn(
            "No existing streaming message for chunk:",
            data.messageId
          );
          return prev;
        }
  ...
  const handleAIMessageComplete = useCallback(
    (data) => {
  ...
  // streamingMessages 관련 selector 추가
  const getStreamingMessagesArray = useMemo(() => {
    return Object.values(streamingMessages);
  }, [streamingMessages]);

  return {
    streamingMessages,
    streamingMessagesArray: getStreamingMessagesArray,
